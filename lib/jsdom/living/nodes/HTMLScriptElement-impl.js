...
function processJavaScript(element, code, filename) {
  ...
  if (window) {
    ...
    // TODO: replace with babel parsing 
    // TODO: implement in proxy
    // we need to preface the code by setting the global window object to _jsdomWindow

    // first make sure no one defined _jsdomWindow. otherwise they are introducing a vulnerability
    if (code.includes("_jsdomWindow")) {
      throw new Error("The global variable _jsdomWindow is reserved for JSDOM's internal use. Please rename your variable.");
    } 

    const globalVariableMappings = [
      {
        oldNames: ["window", "this", "globalThis"],
        newName: "_jsdomWindow"
      },
      {
        oldNames: ["document"],
        newName: "_jsdomDocument"
      },
      {
        oldNames: ["location"],
        newName: "_jsdomLocation"
      },
      {
        oldNames: ["navigator"],
        newName: "_jsdomNavigator"
      }
    ]

    function replaceGlobalObject(code, oldNamesList, newName) {
      // Replace all instances of the global object aliases with the new name.
      if (typeof oldNamesList === "string") {
        oldNamesList = [oldNamesList];
      }
      
      // one regex for all names
      const regex = new RegExp(`\\b(${oldNamesList.join("|")})\\b`, "g");
    ...
    for (const mapping of globalVariableMappings) {
      translatedCode = replaceGlobalObject(translatedCode, mapping.oldNames, mapping.newName);
    }

    try {
      vm.runInContext(translatedCode, window._jsContext, { filename, lineOffset });
    } catch (e) {
      console.error(e);
