...
const DocumentOrShadowRootImpl =
  require("./DocumentOrShadowRoot-impl").implementation;
...
class DocumentImpl extends NodeImpl {
  constructor(globalObject, args, privateData) {
    ...
    this._URL = parsed;
    this._origin =
      urlOption === "about:blank" && privateData.options.parentOrigin
    ...
    this._currentDocumentReadiness =
      privateData.options.readyState || "loading";
    ...
    return NodeList.createImpl(this._globalObject, [], {
      ...
      query: () =>
        domSymbolTree.treeToArray(this, {
          filter: (node) =>
            node.getAttributeNS &&
            node.getAttributeNS(null, "name") === elementName,
        }),
    });
  }
  ...
  get title() {
    ...
    if (documentElement && documentElement._localName === "svg") {
      const svgTitleElement = firstChildWithLocalName(
        documentElement,
        "title",
        SVG_NS
      );
  ...
  get head() {
    return this.documentElement
      ? firstChildWithLocalName(this.documentElement, "head")
      : null;
  }
  ...
  get body() {
    ...
    return firstChildWithLocalNames(
  ...
  createElementNS(namespace, qualifiedName, options) {
    return internalCreateElementNSSteps(
      this,
      namespace,
      qualifiedName,
      options
  ...
  _isFullyActive() {
    ...
    const activeDocument = window.document[idlUtils.implSymbol];

    // Check if it's the active document of its browsing context (JSDOM window)
    if (this !== activeDocument) return false;

    // Now simulate: either top-level or container document is fully active
    // const containerDoc = getContainerDocument(document); // you provide this
    const parentWindow = window.parent == window ? null : window.parent;

    if (!parentWindow) {
      // No container = top-level traversable
      return true;
    }

    // Recurse up the container chain
    return parentWindow.document._isFullyActive();
  }



  // https://encoding.spec.whatwg.org/#names-and-labels
  _getEncodingForLabel(label) {

    // TODO: implement actual encoding detection
    return "UTF-8";
