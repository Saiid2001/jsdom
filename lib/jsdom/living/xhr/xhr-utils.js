...
const proxyUtils = require("../helpers/proxy-utils");
...
function createClient(xhr) {
  ...
  function doRequest() {
    try {
      ...
      // Check if we should use proxy utilities
      const proxyUrl = proxyUtils.getProxyUrl(xhr._globalObject);
      let finalUri = uri;
      let finalHeaders = requestHeaders;

      if (proxyUrl && proxyUtils.shouldProxyUrl(uri)) {
        finalUri = proxyUtils.translateUrlForProxy(uri, proxyUrl);
        finalHeaders = proxyUtils.addProxyHeaders(requestHeaders, uri);
      }

      const requestClient = new Request(finalUri, options, { method: flag.method, headers: finalHeaders });

      // If using proxy, wrap the client to restore original URLs in responses
      if (proxyUrl && proxyUtils.shouldProxyUrl(uri)) {
        const originalEmit = requestClient.emit;
        requestClient.emit = function (event, ...args) {
          if (event === "response") {
            const response = args[0];
            const responseUrl = args[1];

            // Restore original URL for the response
            if (responseUrl && response) {
              const restoredUrl = proxyUtils.restoreOriginalUrl(
                responseUrl,
                proxyUrl,
                (headerName) => response.headers && response.headers[headerName.toLowerCase()]
              );
              args[1] = restoredUrl;
            }
          }
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    // Check if we should use proxy utilities for preflight
    const proxyUrl = proxyUtils.getProxyUrl(xhr._globalObject);
    let preflightUri = uri;
    let preflightHeaders = preflightRequestHeaders;

    if (proxyUrl && proxyUtils.shouldProxyUrl(uri)) {
      preflightUri = proxyUtils.translateUrlForProxy(uri, proxyUrl);
      preflightHeaders = proxyUtils.addProxyHeaders(preflightRequestHeaders, uri);
    }

    const preflightClient = new Request(
      preflightUri,
      { agents, followRedirects: false },
      { method: "OPTIONS", headers: preflightHeaders, rejectUnauthorized }
    );

    // If using proxy for preflight, wrap to restore original URLs
    if (proxyUrl && proxyUtils.shouldProxyUrl(uri)) {
      const originalEmit = preflightClient.emit;
      preflightClient.emit = function (event, ...args) {
        if (event === "response") {
          const response = args[0];
          const responseUrl = args[1];

          // Restore original URL for the preflight response
          if (responseUrl && response) {
            const restoredUrl = proxyUtils.restoreOriginalUrl(
              responseUrl,
              proxyUrl,
    ...
    setTimeout(() => preflightClient.end(), 0);
  } else {
    client = doRequest();
