...
const proxyUtils = require("../../living/helpers/proxy-utils");
...
module.exports = class ResourceLoader {
  constructor({
    ...
    userAgent = `Mozilla/5.0 (${
      process.platform || "unknown OS"
    }) AppleWebKit/537.36 ` + `(KHTML, like Gecko) jsdom/${packageVersion}`,
  } = {}) {
  ...
  static get proxyUtils() {return proxyUtils}
  ...
  fetch(
    urlString,
    { accept, cookieJar, referrer, additionalHeaders, method, body } = {}
  ) {
    ...
    if (!url) {
      return Promise.reject(
        new Error(`Tried to fetch invalid URL ${urlString}`)
      );
    }
    ...
    switch (url.scheme) {
      ...
      case "https": {
        ...
        if (IS_BROWSER) {
          // make sure cors is disabled for navigation requests
        ...
        const promise = new Promise((resolve, reject) => {
          ...
          requestClient.once("response", (res) => {
            ...
            if (statusCode < 200 || statusCode > 299) {
              requestClient.abort();
              reject(
                new Error(`Resource was not loaded. Status: ${statusCode}`)
              );
            }
            ...
            res.on("end", () => resolve(Buffer.concat(accumulated)));
            res.on("error", reject);
          });
        });
      ...
      default: {
        return Promise.reject(
          new Error(
            `Tried to fetch URL ${urlString} with invalid scheme ${url.scheme}`
          )
        );
      }
    }
