...
const {jsdomFetch} = require("../living/fetch/fetch-impl.js");
const settimeoutPolyfill = require("../living/helpers/settimeout-polyfill.js");
...
const customGlobalsInJSContext = [
  "fetch",
  "console",
  "setTimeout",
  "clearTimeout",
...
exports.createWindow = options => {
  ...
  if (makeVMContext) {
    ...
    let _jsContext = {};
    _jsContext = vm.createContext(_jsContext);

    window._globalObject = window;
    window._globalProxy = window;
    window._jsContext = _jsContext;

    _jsContext._jsdomWindow = window;
    ...
    for (const [globalName, globalPropDesc] of jsGlobalEntriesToInstall) {
      const propDesc = { ...globalPropDesc, value: vm.runInContext(globalName, window._jsContext) };
  ...
  if (makeVMContext) {
    window._jsContext._jsdomDocument = window.document;
    window._jsContext._jsdomLocation = window.document.location;
    window._jsContext._jsdomNavigator = window.navigator;
    
    for (const method of customGlobalsInJSContext) {
      window._jsContext[method] = window[method];
    }
  }
...
function installOwnProperties(window, options) {
  ...
  window.fetch = function (input, init={}) {
    return jsdomFetch(window, input, init);
  };

  window.setTimeout = settimeoutPolyfill.setTimeout;
  window.clearTimeout = settimeoutPolyfill.clearTimeout;
  window.setInterval = settimeoutPolyfill.setInterval;
  window.clearInterval = settimeoutPolyfill.clearInterval;
